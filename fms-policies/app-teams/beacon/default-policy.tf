...
module "beacon_default_policy" {
  ...
  # AWS Account scoping
  scope_accounts = [
    "666666666666",  # Beacon Development
    "777777777777"   # Beacon Production
  ]
  ...
  # Default rule groups for all Beacon team resources
  rule_groups = [
    {
      name        = "InternalIPSet"
      priority    = 1
...
resource "aws_wafv2_web_acl_logging_configuration" "beacon_default_policy_logging" {
  log_destination_configs = [var.waf_log_destination_arn]
  resource_arn           = module.beacon_default_policy.policy_arn
